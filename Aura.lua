...
AnimatedJokers = {
    ...
    j_fibonacci = { frames_per_row = 8, frames = 32, fps = 20, credits = {"MRTorres"} },
...
if SMODS.Atlas then
    ...
    local animated_dictionaries = {
        Joker = AnimatedJokers,
        Tarot = AnimatedTarots,
        Planet = AnimatedPlanets,
        Spectral = AnimatedSpectrals,
        Voucher = AnimatedVouchers
    }
    for set, dict in pairs(animated_dictionaries) do
            ...
            if v and next(v) then
                ...
                if Malverk then
                    AltTexture({
                        ...
                        path = k .. ".png",
                        keys = { k },
                        display_pos = k == "j_joker" and k or nil,
                ...
                SMODS[set]:take_ownership(k, {
                    ...
                    animpos = { x = v.pos_x or posx or 0, y = posy or 0, extra = v.extra and {x = v.extra.pos_x or posex or 0, y = v.extra.pos_y or posey or 0, atlas = "aura_"..k.."_extra"} }, --the pos are custom, auromaticly determined or 0,0. The extra dict stores all relevant info for the extra layer. Don't forget the prefix
                    anim = v,
    ...
    if Malverk then
        ...
        --Set default joker as the thumbnail (first in the list)
        for i, key in ipairs(TexturePacks['texpack_aura_animated_jokers'].textures) do
            if key == "aura_j_joker" then
                TexturePacks['texpack_aura_animated_jokers'].textures[i] = TexturePacks['texpack_aura_animated_jokers'].textures[1]
                TexturePacks['texpack_aura_animated_jokers'].textures[1] = key
                break
            end
        end
    end
